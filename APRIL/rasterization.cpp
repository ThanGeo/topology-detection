...
void rasterizeAndIntervalizeNoFloodFill(Polygon &pol, Section &sec){
	...
	//just to save the intervals on disk, move along
	// if(pol.uncompressedF.size() > 0){
	// 	cout << pol.recID << "," << pol.uncompressedF.size() << ",";
	// 	cout << *pol.uncompressedF.begin();
	// 	for(auto it = pol.uncompressedF.begin()+1; it != pol.uncompressedF.end(); it++){		
	// 		cout << "," << *it;
	// 	}
	// 	cout << endl;
	// }
